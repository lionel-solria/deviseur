(function () {
  ...
  function renderProducts() {
    ...
    for (const product of state.filtered) {
      ...
      image.alt = product.name || 'Produit du catalogue';
  ...
  function addToQuote(productId) {
    ...
    if (existing) {
      ...
      state.quote.set(productId, {
        ...product,
        quantity: product.quantityMode === 'area' ? 1 : 1,
  ...
  function generatePdf() {
    ...
    const body = items.map((item) => {
      ...
      const designation = item.comment
    ...
    if (state.generalComment && state.generalComment.trim().length > 0) {
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.text('Commentaire général', margin, nextY);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(11);
      const lines = doc.splitTextToSize(state.generalComment.trim(), pageWidth - margin * 2);
      doc.text(lines, margin, nextY + 18);
      nextY = nextY + 18 + lines.length * 14;
    }

    doc.setFont('helvetica', 'italic');
    doc.setFontSize(11);
    doc.text("Merci pour votre confiance. Ce devis reste modifiable jusqu'à validation écrite.", margin, nextY + 28);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(
